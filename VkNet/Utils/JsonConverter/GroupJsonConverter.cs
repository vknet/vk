...
public class GroupJsonConverter : Newtonsoft.Json.JsonConverter
{
	...
	public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
	{
		...
		var group = new Group
		{
			...
			Type = !response.ContainsKey("type")
				? GroupType.Undefined
				: Utilities.Deserialize<GroupType>(response["type"]),
			...
			Country = !response.ContainsKey("country")
				...
				: JsonConvert.DeserializeObject<Country>(response["country"]
					.ToString()),
			Place = !response.ContainsKey("place")
				? null
				: JsonConvert.DeserializeObject<Place>(response["place"]
					.ToString()),
			...
			Contacts = !response.ContainsKey("contacts")
				? null
				: JsonConvert.DeserializeObject<ReadOnlyCollection<Contact>>(response["contacts"]
					.ToString()),
			Links = !response.ContainsKey("links")
				? null
				: JsonConvert.DeserializeObject<ReadOnlyCollection<ExternalLink>>(response["links"]
					.ToString()),
			...
			MainSection = !response.ContainsKey("main_section")
				? MainSection.NoSection
				: Utilities.Deserialize<MainSection>(response["main_section"]
					.ToString()),
			IsMessagesAllowed = response["is_messages_allowed"],
			Trending = response["trending"],
			CanMessage = response["can_message"],
			Cover = !response.ContainsKey("cover")
				? null
				: JsonConvert.DeserializeObject<GroupCover>(response["cover"]
					.ToString()),
			Market = !response.ContainsKey("market")
				? null
				: JsonConvert.DeserializeObject<GroupMarket>(response["market"]
					.ToString()),
			AgeLimits = !response.ContainsKey("age_limits")
				? AgeLimit.Withoutlimit
				: Utilities.Deserialize<AgeLimit>(response["age_limits"]
					.ToString()),
			MemberStatus = !response.ContainsKey("member_status")
				? MemberStatus.Invited
				: Utilities.Deserialize<MemberStatus>(response["member_status"]
					.ToString()),
			PublicDateLabel = response["public_date_label"],
			Wall = !response.ContainsKey("wall")
				? WallType.Off
				: Utilities.Deserialize<WallType>(response["wall"]
					.ToString())
		};
