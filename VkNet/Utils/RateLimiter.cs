...
public class RateLimiter : IRateLimiter
{
	...
	public async Task Perform(Func<Task> perform, CancellationToken cancellationToken)
	{
		...
		using (await _awaitableConstraint.WaitForReadinessAsync(cancellationToken)
	...
	/// <inheritdoc />
	public void SetRate(int number, TimeSpan timeSpan) => _awaitableConstraint.SetRate(number, timeSpan);
	...
	/// Transformed function.
	/// </returns>
	private static Func<Task<T>> Transform<T>(Func<T> compute) => () => Task.FromResult(compute());
}