	...
	where TFilter : MultivaluedFilter<TFilter>, new()
{
	...
	/// </exception>
	protected static TFilter RegisterPossibleValue(ulong mask, string value) => FromJsonString(val: value);
	...
	/// only one bit must be equal to 1);mask
	/// </exception>
	protected static TFilter RegisterPossibleValue(string value) => FromJsonString(val: value);
	...
	public static TFilter operator |(MultivaluedFilter<TFilter> a, MultivaluedFilter<TFilter> b) => new()
	{
		Selected = a.Selected.Union(second: b.Selected)
