...
public partial class Browser : IBrowser
{
	...
	/// <inheritdoc />
	public string GetJson(string url, IEnumerable<KeyValuePair<string, string>> parameters) => GetJsonAsync(url, parameters)
		.ConfigureAwait(false)
		.GetAwaiter()
		.GetResult();

	/// <inheritdoc />
	public void SetAuthorizationParams(IApiAuthParams authorizationParams) => _authParams = authorizationParams;
	...
	private static bool IsAuthSuccessful(HttpResponse<string> webCallResult) =>
		UriHasAccessToken(webCallResult.RequestUri) || UriHasAccessToken(webCallResult.ResponseUri);
