...
internal sealed partial class WebCall
{
														...
														IWebProxy webProxy)
	{
		using (var call = new WebCall(url, new(), webProxy))
		{
			...
			return await call.MakeRequestAsync(request, new(url), webProxy)
				.ConfigureAwait(false);
		}
	}
	...
	public static async Task<WebCallResult> PostAsync(WebForm form, IWebProxy webProxy)
	{
		using (var call = new WebCall(form.ActionUrl, form.Cookies, webProxy, false))
		{
			...
			return await call.MakeRequestAsync(request, new(form.ActionUrl), webProxy)
				.ConfigureAwait(false);
		}
	}
	...
	private async Task<WebCallResult> MakeRequestAsync(HttpResponseMessage response, Uri uri, IWebProxy webProxy)
	{
		...
		if (IsAbsoluteUrl(response.Headers.Location?.ToString()))
		{
			return response.StatusCode == HttpStatusCode.Redirect
				? await RedirectToAsync(response.Headers.Location?.AbsoluteUri, webProxy)
					.ConfigureAwait(false)
				: _result;
		}
	...
	private static bool IsAbsoluteUrl(string url) => !string.IsNullOrWhiteSpace(url) && Uri.TryCreate(url, UriKind.Absolute, out var _);

	private string GetDomain(Uri uri) => uri.GetLeftPart(UriPartial.Authority);
}