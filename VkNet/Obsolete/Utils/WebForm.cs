...
internal sealed class WebForm
{
	...
	public string ActionUrl
	{
		get {
			...
			var link = formNode.Attributes["action"]
				.Value;
	...
	public static WebForm From(HttpResponse<string> result) => new(result);
	...
	/// <param name="name"> Наименование поля. </param>
	/// <returns> WEB форма. </returns>
	public string GetFieldValue(string name) => _inputs.TryGetValue(name, out var result)
		? result
		: default;
	...
	public byte[] GetRequest() => Encoding.UTF8.GetBytes(GetRequestAsStringArray()
		.JoinNonEmpty("&"));
