...
public partial class AccountCategory
{
	...
	/// <inheritdoc />
	public Task<bool> SetOfflineAsync(CancellationToken token = default) => TypeHelper.TryInvokeMethodAsync(SetOffline, token);
	...
	public Task<bool> BanUserAsync(long ownerId,
									CancellationToken token = default) => BanAsync(ownerId, token);
	...
	public Task<bool> UnbanUserAsync(long ownerId,
									CancellationToken token = default) => UnbanAsync(ownerId, token);
