...
namespace VkNet.Categories
{
	...
	public partial class MessagesCategory : IMessagesCategory
	{
        ...
        public VkCollection<Message> GetById(IEnumerable<ulong> messageIds, uint? previewLength = null)
        {
            if (!messageIds.Any())
            {
                throw new System.Exception("messageIds не может быть пустой");
            }
		...
		{
			return _vk.Call("messages.markAsAnsweredDialog", new VkParameters
			{
				{"peer_id", peerId},
				{"answered", answered}
			});
		...
		{
			return _vk.Call("messages.markAsImportantDialog", new VkParameters
			{
				{"peer_id", peerId},
				{"important", important}
			});
