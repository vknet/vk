...
public class ClientInfo
{
	...
	/// <returns> </returns>
	public static ClientInfo FromJson(VkResponse response) => new()
	{
		ButtonActions = response["button_actions"]
			.ToReadOnlyCollectionOf<KeyboardButtonActionType>(x => x),
		Keyboard = response["keyboard"],
		InlineKeyboard = response["inline_keyboard"],
		LangId = response["lang_id"]
			.ToEnum<Language>()
	};
