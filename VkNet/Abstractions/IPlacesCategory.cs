...
namespace VkNet.Abstractions
{
    ...
    public interface IPlacesCategory : IPlacesCategoryAsync
    {
        ...
        /// </summary>
        /// <param name = "places">
        /// Перечисленные через запятую идентификаторы мест список положительных чисел, разделенных запятыми, обязательный параметр
        /// </param>
        /// <returns>
        /// После успешного выполнения возвращает массив объектов place, каждый из которых имеет следующие поля:
        ...
        /// Также, при наличии соответствующей информации, в объект могут включаться поля checkins, updated, type, country, city, address.
        /// </returns>
        /// <remarks>
        ...
        /// </param>
        /// <returns>
        /// После успешного выполнения возвращает количество отметок и массив объектов chekin, каждый из которых имеет следующие поля:
        ...
        /// Если в запросе был задан параметр need_places, то в объекте могут дополнительно содержаться следующие поля:
        ...
        /// place_icon.
        /// Если не заданы параметры latitude и longitude или параметр place, то будет возвращена лента обновлений отметок друзей. При указании параметра timestamp будут возвращены только те отметки из ленты обновлений, которые были созданы после указанного значения timestamp. Если timestamp не указан, то отметки из ленты будут возвращены с учетом параметров offset и count (если они заданы).
        /// Если заданы параметры latitude и longitude, то будут возвращены все отметки, сделанные за последние 24 часа, координаты которых находятся недалеко от Вас. Если задан параметр friends_only, то будут возвращены только последние ближайшие отметки Ваших друзей.
        /// Если задан параметр place, то будут возвращены последние 20 отметок, сделанные в месте с идентификатором, указанном в параметре place.
        /// Если задан параметр uid, то будут возвращены последние 20 отметок, сделанные пользователем с идентификатором, указанном в параметре uid.
        /// </returns>
        /// <remarks>
        ...
        /// </summary>
        /// <returns>
        /// Возвращает массив всех возможных типов мест, каждый из объектов которого содержит поля tid, title и icon.
        /// </returns>
        /// <remarks>
        ...
        /// </param>
        /// <returns>
        /// После успешного выполнения возвращает общее количество найденных мест и массив объектов place, каждый из которых имеет следующие поля:
        /// id — идентификатор места;
        /// title — название места;
        /// latitude — географическая широта, заданная в градусах (от -90 до 90);
        /// longitude — географическая долгота, заданная в градусах (от -180 до 180);
        ...
        /// Если место прикреплено к сообществу, дополнительно возвращаются поля group_id и group_photo.
