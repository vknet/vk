...
namespace VkNet.Tests
{
    ...
    public abstract class BaseTest //TODO: V3072 http://www.viva64.com/en/w/V3072 The 'BaseTest' class containing IDisposable members does not itself implement IDisposable. Inspect: Api.
    {
        ...
        public void Init()
        {
            ...
            restClient.Setup(x => x.PostAsync(It.Is<Uri>(s => s == new Uri(Url)), It.IsAny<IEnumerable<KeyValuePair<string, string>>>()))
                .Callback(Callback)
                .Returns(() =>
                {
                    if (string.IsNullOrWhiteSpace(Json))
            ...
            restClient.Setup(x => x.PostAsync(It.Is<Uri>(s => string.IsNullOrWhiteSpace(Url)), It.IsAny<IEnumerable<KeyValuePair<string, string>>>()))
