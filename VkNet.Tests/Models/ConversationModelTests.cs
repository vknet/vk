...
namespace VkNet.Tests.Models
{
	...
	public class ConversationModelTests : BaseTest
	{
		...
		public void ConversationModel()
		{
			...
			conversation.CanWrite.Allowed.Should().BeTrue();
			conversation.CanReceiveMoney.Should().BeTrue();
			...
			conversation.ChatSettings.Acl.CanChangeInfo.Should().BeTrue();
			conversation.ChatSettings.Acl.CanChangeInviteLink.Should().BeFalse();
			conversation.ChatSettings.Acl.CanChangePin.Should().BeTrue();
			conversation.ChatSettings.Acl.CanInvite.Should().BeTrue();
			conversation.ChatSettings.Acl.CanPromoteUsers.Should().BeFalse();
			conversation.ChatSettings.Acl.CanSeeInviteLink.Should().BeFalse();
