...
namespace VkNet.Tests.Infrastructure
{
	...
	public class AuthorizationFormHtmlParserTests : BaseTest
	{
		...
		public async Task ConsentForm()
		{
			...
			Assert.NotNull(result);
			Assert.AreEqual("post", result.Method);
			Assert.AreEqual("https://login.vk.com/?act=grant_access&amp;client_id=4268118&amp;settings=140492255&amp;redirect_uri=https%3A%2F%2Foauth.vk.com%2Fblank.html&amp;response_type=token&amp;group_ids=&amp;token_type=0&amp;v=&amp;state=123&amp;display=mobile&amp;ip_h=5a4524d95f3521be68&amp;hash=1552162134_98614f6fce5d86d252&amp;https=1",
							result.Action);
			Assert.IsNotEmpty(result.Fields);
		}
